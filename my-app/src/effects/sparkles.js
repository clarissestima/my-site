// Tinkerbell Magic Sparkle Effect
...
export function initSparkles() {
  ...
  for (let i = 0; i < sparkleConfig.sparkles; i++) {
...
function sparkle() {
  if (Math.abs(x - ox) > 1 || Math.abs(y - oy) > 1) {
    ox = x;
    oy = y;
    for (let c = 0; c < sparkleConfig.sparkles; c++) {
      if (!starv[c]) {
        star[c].style.left = (starx[c] = x) + "px";
        star[c].style.top = (stary[c] = y + 1) + "px";
        star[c].style.clip = "rect(0px, 5px, 5px, 0px)";
        const color = sparkleConfig.colour === "random" ? newColour() : sparkleConfig.colour;
